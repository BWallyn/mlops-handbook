...
Select your virtual environment and then use `uv` to install MLflow:
```bash
...
def train_classifier(
    ...
    Args:
        ...
        target (pd.Series): Binary target vector.
        params (dict[str, Any]): Hyperparameters. Must contain
            'n_estimators', 'max_depth', 'random_state', and
            'test_size'.

    Returns:
        model (RandomForestClassifier): A trained random forest model.
        metrics (dict[str, float]): Metrics of the model on the
            validation set.
    """
...
Keep all hyperparameters in a yaml file (for example `conf/base/parameters/experiment_tracking.yml`). Never hardcode them in the training script.
