...
* A **conf/** directory, which contains all the parameters and catalog yaml files.
  * The **parameters_<name_pipeline>.yml** files, one file per pipeline which contains the parameters used by this pipeline.
...
Kedro is an important tool to structure your repository. However, we advise you to follow several steps to have the easiest experience using it:
1. The first step is to define the overall architecture of your repository, in order to think of the different kedro pipelines to create.
2. Then, test your intuitions, develop functions and work on your pipeline in a notebook. Think of the different parameters of the functions you might want to change and you will store in the parameters files.
3. Once you are satisfied with your pipeline, you can implement it in your Kedro pipeline. Put the functions in the nodes.py file, write the architecture in the pipeline.py file, parameters in the parameters_<name_pipeline>.yml file and the inputs and outputs in the catalog.yml file.